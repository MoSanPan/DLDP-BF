...
for m in m_values:
    ...
    # 计算当前 m 的平均 RMSE 和 MAE
    avg_rmse = np.mean(rmse_list)
    m_avg_rmse[m] = avg_rmse

    avg_mae = np.mean(mae_list)
    m_avg_mae[m] = avg_mae

    # 输出当前 m 的平均 RMSE 和 MAE
    print(f"m = {m}: 平均 RMSE = {avg_rmse:.4f}")
    print(f"m = {m}: 平均 MAE = {avg_mae:.4f}")

# 输出所有 m 对应的平均 RMSE 和 MAE
print("\n所有 m 对应的平均 RMSE 和 MAE:")
for m in m_values:
    print(f"m = {m}: 平均 RMSE = {m_avg_rmse[m]:.4f}, 平均 MAE = {m_avg_mae[m]:.4f}")
